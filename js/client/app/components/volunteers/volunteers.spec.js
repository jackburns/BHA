...
import VolunteersTemplate from './volunteers.html';
import _ from 'lodash';

describe('Volunteers', () => {
  let $rootScope, makeController;

  beforeEach(window.module(VolunteersModule.name));
  beforeEach(inject((_$rootScope_) => {
    $rootScope = _$rootScope_;
    makeController = () => {
      return new VolunteersController();
    };
  }));
  ...
  describe('Component', () => {
    ...
    it('includes the intended template',() => {
      expect(component.template).to.equal(VolunteersTemplate);
    });
    ...
    it('invokes the right controller', () => {
      expect(component.controller).to.equal(VolunteersController);
    });
  });
});
